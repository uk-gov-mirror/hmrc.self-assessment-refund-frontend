...
import uk.gov.hmrc.selfassessmentrefundfrontend.bars.model.response.BarsAssessmentType._
...
object VerifyResponse {
  ...
  object nameMatchesNo {
    def unapply(response: VerifyResponse): Boolean = {
      val resp = response.barsVerifyResponse
      resp.nameMatches =!= Yes && resp.accountExists === Yes
    }
  }

  object nonStandardDetailsRequiredYes {
    def unapply(response: VerifyResponse): Boolean = {
      val resp = response.barsVerifyResponse
      resp.nonStandardAccountDetailsRequiredForBacs === Yes
    }
  }
  ...
  object sortCodeSupportsDirectCreditNo {
    def unapply(response: VerifyResponse): Boolean =
      response.barsVerifyResponse.sortCodeSupportsDirectCredit === No
  }
