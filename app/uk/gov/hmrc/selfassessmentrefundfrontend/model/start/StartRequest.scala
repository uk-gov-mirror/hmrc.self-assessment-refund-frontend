...
package uk.gov.hmrc.selfassessmentrefundfrontend.model.start

import play.api.libs.json.{Json, OFormat, OWrites, Writes}
import uk.gov.hmrc.selfassessmentrefundfrontend.model.ReturnUrl

sealed trait StartRequest extends Product with Serializable {
  val nino: String
...
object StartRequest {
  ...
  final case class ViewHistory(nino: String) extends StartRequest {
    val replyUrl = "/self-assessment-refund/refund-history"
  }
