...
import uk.gov.hmrc.selfassessmentrefundfrontend.views.html.refundrequestjourney.WeNeedBankDetailsPage
...
class WeNeedBankDetailsController @Inject() (
  ...
  val onPageLoad: Action[AnyContent] = actions.authenticatedRefundJourneyAction.async { implicit request =>
    Future.successful(Ok(weNeedBankDetailsPage(isAgent = false)))
  }

  val onPageLoadAgent: Action[AnyContent] = actions.authenticatedRefundJourneyAction.async { implicit request =>
    Future.successful(Ok(weNeedBankDetailsPage(isAgent = true)))
  }
