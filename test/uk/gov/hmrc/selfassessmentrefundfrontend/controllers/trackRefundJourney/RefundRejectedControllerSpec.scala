...
class RefundRejectedControllerSpec extends ItSpec with PageContentTesting {
  ...
  "onPageLoad" should {
    "render 'Refund Rejected' page with correct content" in new TestSetup() {
      ...
      result.checkPageIsDisplayed(
        ...
        contentChecks       = checkRejectedPageContent,
    ...
    "render 'Refund Rejected' page with correct content for Agent" in new TestSetup(isAgent = true) {
      ...
      result.checkPageIsDisplayed(
        expectedHeading     = "Your refund of £12,000 has been rejected",
        expectedServiceLink = "http://localhost:9171/track-a-self-assessment-refund/refund-request-tracker",
        contentChecks       = checkAgentRejectedPageContent,
        expectedStatus      = Status.OK,
        journey             = "track"
      )
    }
  ...
  private def checkRejectedPageContent(doc: Document): Unit = {

    doc.checkHasParagraphs(rejectedParagraphs)

    doc.checkHasHyperlink("try again to request a refund", "http://localhost:9081/report-quarterly/income-and-expenses/view/claim-refund")

    doc.checkHasBackLinkWithUrl("#")
  }

  private def checkAgentRejectedPageContent(doc: Document): Unit = {

    doc.checkHasParagraphs(rejectedParagraphs)

    doc.checkHasHyperlink("try again to request a refund", "http://localhost:9081/report-quarterly/income-and-expenses/view/agents/claim-refund")
