...
class JourneyConnectorSpec extends ItSpec {
  ...
  val startRefundRequest: StartRequest.StartRefund =
    StartRequest.StartRefund("AA999999A", BigDecimal(987.611), None, None)
  ...
  "start" should {
    "return a ReplyURL for the start refund journey" in {
      stubFor(
        post(urlEqualTo(startUrl(isHistory = false)))
          .willReturn(
            aResponse()
              .withStatus(OK)
    ...
    "return a ReplyURL for the view history journey" in {
      stubFor(
        post(urlEqualTo(startUrl(isHistory = true)))
          .willReturn(
            aResponse()
              .withStatus(OK)
              .withBody(Json.toJson(replyUrl).toString())
